...
Write as if you deeply understand {audience} and their specific challenges with {description}. Be authentic, specific, and never template-like. Reference the actual product name "{campaign}" and speak directly to {audience} needs."""
            ...
            content = self.generate_with_llama(prompt, max_tokens=300, timeout=8, max_retries=1)
            ...
            if content and len(content.strip()) >= 100:
                ...
                # Smart fallback when AI is unavailable - still professional content
                print(f"[CONTENT GENERATION] AI unavailable, using smart template for {self.name}")
                
                # Generate smart template based on platform and funnel stage
                fallback_content = self.generate_smart_fallback(task_data)
